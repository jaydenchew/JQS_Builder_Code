...
## DD-028: Display Timezone Toggle Is Browser-Local + Reports Page Shares the Same Read Path

**What**: A new GMT+7/+8 nav toggle lets operators switch how dates are interpreted in display-side code paths (today boundary, date filter parsing, timestamp formatting). Backed by `localStorage` (per-browser) and a `displaytz:changed` custom event. The new `/reports` page consumes the same `?tz=` parameter on a dedicated `/api/monitor/reports/summary` endpoint that returns five aggregated sections (per-arm, per-bank, top failing steps, stall reasons, slowest steps).

**Why this design over alternatives**:

- **localStorage, not a per-user DB column**: the toggle is a viewing preference, not an operational setting. Persisting it per-browser means: (a) no schema change, (b) different browsers / kiosks can show different views simultaneously, (c) a Singapore visitor switches to GMT+8, leaves, and the regular Cambodia operator sees the dashboard in GMT+7 unchanged. A DB-backed user setting would force a synchronization step on every page load and surface a multi-user coordination problem we don't actually have.
- **Query param `?tz=7|8`, not a request header or cookie**: query params are easy to inspect in browser devtools, easy to share via URL ("look at this report — `/reports?date_from=…&tz=8`"), and don't require middleware. A header would be invisible in the network tab unless devtools is open; a cookie would auto-send on every API call (including ones that don't care) and would have to be cleaned up.
- **Whitelist `{7, 8}` with fallback to 7, not arbitrary tz**: the system serves Cambodia (GMT+7) and occasionally cross-checks with GMT+8 visitors. Accepting any int would invite bug reports about edge cases (-12, 25, fractional offsets) and add a real `pytz`/`zoneinfo` dependency on what's currently a 2-hour difference. The whitelist is a deliberate scope cap — easy to expand to 3-4 values later, not worth genericizing now.
- **Runtime path stays UTC, only display path varies**: every `datetime.now(timezone.utc)` call in `arm_worker.py`, `actions.py`, `pas_client.py`, etc. is unchanged. The TZ toggle exclusively affects two read endpoints (`/stats/today`, `/transactions`) and one new read endpoint (`/reports/summary`). The `_resolve_display_tz` helper is only ever called inside those endpoints. This guarantees that toggling cannot affect transaction processing, callbacks, stall handling, or anything else — verifiable by grep alone.
- **Frontend fallback when `per_arm` is missing**: `static/index.html` checks `lastPerArm !== null` before showing `Today: success/total`, falling back to `Completed: ${a.task_count}` (the worker counter) when the backend hasn't been restarted yet. This avoids the "Today: 0 on every card" UX bug during the rolling-deploy window where new frontend talks to old backend.
- **Chart.js bundled locally, not via CDN**: matches the project's existing convention — `nssm.exe`, `tesseract-setup.exe`, `cloudflared.exe` are all checked into `deploy/`, not downloaded at install time. The deployment target is a Windows machine that may have intermittent / firewall-restricted internet, so an offline-first install is the safer default. Cost: +200 KB to the repo. Benefit: `/reports` works with no network access.
- **Single `/reports/summary` endpoint, five sections in one response**: the alternative (one HTTP call per section) would mean five round-trips per page load, each with its own date-range / tz / arm-id parameters to keep in sync. One endpoint with one set of parameters is simpler to call from the page (one `await`) and simpler to test (one fixture). The five queries inside the endpoint are independent SELECTs against indexed columns; a date range of 7 days returns in <100 ms in our typical data volume.
- **Five sections, not three or seven**: per-arm + per-bank + top-failing-steps is the analytical core (where + what is broken). Stall-reasons distribution is the next concrete signal (drives the OCR / CHECK_SCREEN / find-and-click prioritization decisions made in DD-024 / 025 / 026). Slowest-steps is the final natural axis (latency vs. correctness — the only one operators routinely ask about beyond success rate). Beyond five (e.g. hourly distribution, per-transfer-type comparison) crosses into "report builder" territory we don't yet need.
- **Read-only endpoint, not WebSocket**: dashboard's existing 2 s WS push is for live arm status. The reports page is a snapshot view that operators refresh manually after changing filter inputs. Pushing report aggregations on every WS tick would burn CPU on data the operator isn't watching second-by-second. A pull-on-demand HTTP endpoint matches actual usage.
- **Quick-filter buttons (Today / Yesterday / Last 7 / Last 30) compute the day in `getDisplayTZ()`**: clicking "Today" while toggled to GMT+8 sets the date input to GMT+8's today, not GMT+7's. Otherwise the operator's mental model breaks ("I clicked Today in GMT+8 view but the data shows yesterday in some places"). Implemented by shifting the JS `Date.now()` by the offset before formatting as `YYYY-MM-DD`.

**Trade-offs accepted**:

- The whitelist `{7, 8}` is a hardcoded value pair. If the project ever needs to support a third timezone (e.g. KH operations open a Vietnamese branch), this would require extending the whitelist and adding the option to the nav toggle. Acceptable because it's a single line change in two places (`api.js` toggle handler and `monitor.py` whitelist), with no schema migration needed.
- Per-arm / per-bank stat aggregations are computed on each page load (no caching). For our data volume (low-thousands of rows per day) this is fine; for a 6-month-old DB with 100k+ rows the queries would still be sub-second thanks to indexes on `created_at` and `station_id`. If volume grows by 10x, we'd add a materialized daily-summary table; not pre-built.
- "Today: 0" fallback covers the `per_arm` field but not the per-arm AVG duration field — old service responses simply don't include `avg_duration_s`, and the dashboard arm card doesn't show duration anyway. Reports page shows "—" for missing duration. Acceptable.
- The `displaytz:changed` event is dispatched as a `CustomEvent` on `window`; pages that don't listen (e.g. `/recorder`, `/settings`) won't refresh anything but also don't break — they ignore it. No global pub-sub framework, no state-management library — just the browser's native event system.

**What this does NOT change**:

- Any worker / arm / camera / OCR / flow execution code path. Verifiable by grep: no `monitor.py` import in `arm_worker.py`, `actions.py`, etc.
- DB schema, seed files, transaction-handling SQL.
- Existing API contracts (`/process-withdrawal`, PAS callbacks, etc.).
- The dashboard's WebSocket push frequency, payload shape, or worker counter semantics (`a.task_count` is still a valid field in the WS message; the dashboard just chooses not to display it when `per_arm` is available).
